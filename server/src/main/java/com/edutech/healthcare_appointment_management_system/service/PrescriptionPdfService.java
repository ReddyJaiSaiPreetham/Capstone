...
public class PrescriptionPdfService {
    ...
    public byte[] generatePrescriptionPdf(MedicalRecord record) {
        ...
        try (PDDocument document = new PDDocument();
            ...
            try (PDPageContentStream content = new PDPageContentStream(document, page)) {
                ...
                y = writeLine(content, "HEALTHCARE APPOINTMENT MANAGEMENT SYSTEM", margin, y);
                ...
                float col1 = 140; // Medicine
                float col2 = 70;  // Dosage
                float col3 = 70;  // Frequency
                float col4 = 40;  // Days
                float col5 = tableWidth - (col1 + col2 + col3 + col4); // Instructions
                ...
                if (items == null || items.isEmpty()) {
                    ...
                    for (PrescriptionItem p : items) {
                        ...
                        if (y < 80) {
                            content.endText(); // safe end if any text open
                            // For simplicity: we keep single-page for now
                            // If you want multipage, we can add next page logic later.
                            break;
                        }
                    }
                }
                ...
                writeLine(content, "Generated by HAM System - For hospital use only", margin, y);
            }
    ...
    private float writeWrapped(PDPageContentStream content, String text, float x, float y, float maxWidth) throws IOException {
        ...
        for (String word : words) {
            ...
            // rough width estimation (works fine for Helvetica sizes here)
            float size = 11;
